...
//		return (new FisherExact((int) ct.getSum())).getTwoTailedP(matrix[0][0], matrix[0][1], matrix[1][0], matrix[1][1]);
		
		for (int i = 19; i < 20; i++) {
			int sum = 10*i;
			double max = 0;
			double min = 1;
			FisherExact fe = new FisherExact(sum+10);
			for (int a = 6; a < sum - 12; a++) {
				for (int b = 6; b < (sum -a - 6); b++) {
					int c = sum -a -b;
					double d = fe.getTwoTailedP(5, a, b, c);
					max = Math.max(max, d);
					min = Math.min(min, d);
					if (Double.compare(min, 0.01) <= 0) {
